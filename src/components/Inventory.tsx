...
const Library = () => {
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(collection(db, "productsNane"), (snapshot) => {
      setProducts(
  ...
  const handleAddProduct = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await addDoc(collection(db, "productsNane"), {
        ...newProduct,
        image: imageUrl,
  ...
  const handleDeleteProduct = async (id: string) => {
    ...
    try {
      await deleteDoc(doc(db, "gioproducts", id));
    } catch (error) {
  ...
  const handleToggleShow = async (id: string, currentShow: boolean) => {
    try {
      const productRef = doc(db, "gioproducts", id);
